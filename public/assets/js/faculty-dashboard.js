(function () {
    ...
    window.detectCurrentLocation = async function () {
        ...
        try {
            ...
            document.getElementById('classLatitude').value = latitude.toFixed(6);
            document.getElementById('classLongitude').value = longitude.toFixed(6);

            showToast(`Location detected: ${latitude.toFixed(4)}, ${longitude.toFixed(4)}`, 'success');
        } catch (error) {
            console.error('Geolocation error:', error);
            showToast('Failed to detect location. Please enable GPS and try again.', 'error');
        } finally {
            btn.innerHTML = originalText;
            btn.disabled = false;
        }
    ...
    // Start WiFi attendance session
    window.startAttendanceSession = async function () {
        const latitude = parseFloat(document.getElementById('classLatitude').value);
        const longitude = parseFloat(document.getElementById('classLongitude').value);
        const room = document.getElementById('classRoom').value || 'Room-' + Date.now();

        if (!latitude || !longitude) {
            showToast('Please set classroom location first', 'warning');
            return;
        }
        ...
        try {
            // Generate WiFi SSID
            const wifiSSID = `ATTEND_${user.name.split(' ')[0].toUpperCase()}_${Math.random().toString(36).substr(2, 9)}`;

            // Call API to start session
            const response = await apiRequest('/attendance/session/start', {
                method: 'POST',
                body: JSON.stringify({
                    timetableId: 'current',
                    subjectId: null,
            ...
            currentAttendanceSession = response.session;
            studentStatusMap.clear();

            // Update UI
            document.getElementById('sessionStatusPanel').style.display = 'block';
            document.getElementById('locationSetupForm').style.display = 'none';
            document.getElementById('startSessionBtn').style.display = 'none';
            document.getElementById('stopSessionBtn').style.display = 'inline-block';

            document.getElementById('sessionIdDisplay').textContent = currentAttendanceSession._id;
            document.getElementById('wifiSSIDDisplay').textContent = wifiSSID;
            document.getElementById('locationDisplay').textContent = `${latitude.toFixed(4)}, ${longitude.toFixed(4)} (${room})`;

            // Emit socket event
            if (typeof socket !== 'undefined') {
                socket.emit('sessionStarted', {
                    sessionId: currentAttendanceSession._id,
                    facultyId: user.id,
                    facultyName: user.name,
                    subjectName: 'Current Class',
                    room,
                    wifiSSID,
                ...
                socket.on('studentWifiConnected', (data) => {
                    updateStudentStatus(data.studentId, 'wifi_connected', data);
                });

                socket.on('studentLocationVerified', (data) => {
                    updateStudentStatus(data.studentId, 'location_verified', data);
                });

                socket.on('studentFaceVerified', (data) => {
                    updateStudentStatus(data.studentId, 'face_verified', data);
                });

                socket.on('attendanceRecorded', (data) => {
                    updateStudentStatus(data.studentId, 'marked', data);
                });
            }
            ...
            btn.innerHTML = '<i class="bi bi-play-fill me-2"></i>Start WiFi Session';
            btn.disabled = false;
        }
    };

    // Stop attendance session
    window.stopAttendanceSession = async function () {
        ...
        try {
            const response = await apiRequest(`/attendance/session/${currentAttendanceSession._id}/end`, {
                method: 'POST',
                body: JSON.stringify({
                    statistics: {
                        totalStudents: studentStatusMap.size,
                        presentCount: Array.from(studentStatusMap.values()).filter(s => s.status === 'marked').length,
                        absentCount: Array.from(studentStatusMap.values()).filter(s => s.status !== 'marked').length
                    }
                })
            });
            ...
            // Reset UI
            document.getElementById('sessionStatusPanel').style.display = 'none';
            document.getElementById('locationSetupForm').style.display = 'block';
            ...
            btn.innerHTML = '<i class="bi bi-stop-fill me-2"></i>Stop Session';
            btn.disabled = false;
        }
    };

    // Update student status
    function updateStudentStatus(studentId, status, data = {}) {
        const student = studentStatusMap.get(studentId) || {
            studentId,
        ...
        // Status progression: wifi_connected -> location_verified -> face_verified -> marked
        const statusLevels = ['wifi_connected', 'location_verified', 'face_verified', 'marked'];
        const currentLevel = statusLevels.indexOf(student.status);
        ...
        if (status === 'location_verified') {
            student.distance = Math.round(data.distance || 0);
        }

        studentStatusMap.set(studentId, student);
        renderStudentStatusList();
        updateSessionStats();
    }

    // Render student status list
    function renderStudentStatusList() {
        ...
        const studentSearch = document.getElementById('studentSearchFilter')?.value?.toLowerCase() || '';

        const filteredStudents = Array.from(studentStatusMap.values()).filter(s =>
            s.studentName.toLowerCase().includes(studentSearch) ||
            s.rollNumber.toLowerCase().includes(studentSearch)
        );

        tbody.innerHTML = filteredStudents.map(student => {
            const statusBadges = {
                'wifi_connected': '<span class="badge bg-info">📡 WiFi</span>',
                'location_verified': '<span class="badge bg-info">📍 Location</span>',
                'face_verified': '<span class="badge bg-warning">👤 Face Verifying</span>',
                'marked': '<span class="badge bg-success">✅ Marked</span>'
            };

            const wifiIcon = student.status ? '✓' : '✗';
            const locationIcon = ['location_verified', 'face_verified', 'marked'].includes(student.status) ? '✓' : '✗';
            const faceIcon = ['face_verified', 'marked'].includes(student.status) ? '✓' : '✗';

            return `
                <tr>
                    <td><strong>${student.studentName}</strong></td>
                    <td>${student.rollNumber}</td>
                    <td class="text-center">${wifiIcon}</td>
                    <td class="text-center">${locationIcon}</td>
                    <td class="text-center">${faceIcon}</td>
                    <td>${statusBadges[student.status] || 'Pending'}</td>
                    <td><small>${student.markedAt ? new Date(student.markedAt).toLocaleTimeString() : '-'}</small></td>
                    <td><small>${student.distance ? student.distance + 'm' : '-'}</small></td>
                </tr>
            `;
        }).join('');
    }
    ...
    function updateSessionStats() {
        const statuses = Array.from(studentStatusMap.values()).map(s => s.status);

        document.getElementById('totalStudentsCount').textContent = studentStatusMap.size;
        document.getElementById('presentStudentsCount').textContent = statuses.filter(s => s === 'marked').length;
        document.getElementById('verifyingStudentsCount').textContent = statuses.filter(s => s !== 'marked').length;
        document.getElementById('absentStudentsCount').textContent = Math.max(0, (allStudents.length || 0) - studentStatusMap.size);
    }

    // Refresh student status from server
    window.refreshStudentStatus = async function () {
        if (!currentAttendanceSession) return;

        try {
            const response = await apiRequest(`/attendance/session/${currentAttendanceSession._id}/students-status`);

            if (response.success) {
                // Update student status map from server
                response.students.forEach(student => {
                    studentStatusMap.set(student.studentId, {
                        studentId: student.studentId,
                        studentName: student.studentName,
                        rollNumber: student.rollNumber,
                        status: student.status === 'present' || student.status === 'late' ? 'marked' : 'wifi_connected',
                        markedAt: student.markedAt,
                        distance: student.verifications?.distance
                    });
                });
            ...
            console.error('Failed to refresh student status:', error);
        }
    };
    ...
    function connectToSocket() {
        ...
        try {
            ...
            socket.on('connect', () => {
                ...
                if (currentAttendanceSession) {
                    socket.emit('joinSession', currentAttendanceSession._id);
                }
            });
