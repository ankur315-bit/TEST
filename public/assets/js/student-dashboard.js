(function () {
    ...
    window.onload = async function () {
        ...
        // Safety: remove any stray modal backdrop that could block clicks
        document.querySelectorAll('.modal-backdrop').forEach(el => el.remove());
        document.body.classList.remove('modal-open');

        // Refresh user from backend to ensure the correct student profile
        try {
    ...
    // LOAD ATTENDANCE SECTION WITH TODAY'S CLASSES
    function loadAttendanceSection() {
        ...
        const classList = document.getElementById('attendanceClassesList');
        if (classList) {
            ...
            todayClasses.forEach((cls, idx) => {
                const now = new Date();
                const nowMinutes = now.getHours() * 60 + now.getMinutes();
                const classStart = cls.t ? parseInt(cls.t.split(':')[0]) * 60 + parseInt(cls.t.split(':')[1]) : 0;
                const classEnd = cls.endTime ? parseInt(cls.endTime.split(':')[0]) * 60 + parseInt(cls.endTime.split(':')[1]) : classStart + 60;

                let statusClass = '';
                let statusBadge = '';
                let isOngoing = nowMinutes >= classStart && nowMinutes < classEnd;
                let isPending = nowMinutes < classStart;
                let isCompleted = nowMinutes >= classEnd;

                if (cls.st === 'Present') {
                    statusClass = 'marked';
                    statusBadge = '<span class="badge bg-success ms-2"><i class="bi bi-check-circle me-1"></i>Marked</span>';
                } else if (isOngoing) {
                    statusClass = 'ongoing';
                    statusBadge = '<span class="badge bg-warning ms-2"><i class="bi bi-exclamation-circle me-1"></i>Ongoing</span>';
                } else if (isPending) {
                    statusClass = 'pending';
                    statusBadge = '<span class="badge bg-info ms-2"><i class="bi bi-clock me-1"></i>Upcoming</span>';
                } else if (isCompleted) {
                    statusClass = 'missed';
                    statusBadge = '<span class="badge bg-danger ms-2"><i class="bi bi-x-circle me-1"></i>Missed</span>';
                }

                html += `
                    <div class="attendance-class-card ${statusClass} mb-3 p-3 border rounded-3">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <div>
                                <h5 class="mb-1"><i class="bi bi-book me-2" style="color: var(--primary-purple);"></i>${cls.s || 'Subject'}</h5>
                                <p class="text-muted small mb-0">
                                    <i class="bi bi-clock me-1"></i>${cls.t} - ${cls.endTime}
                                    ${cls.r ? `<br><i class="bi bi-geo-alt me-1"></i>${cls.r}` : ''}
                                    ${cls.faculty ? `<br><i class="bi bi-person me-1"></i>${cls.faculty}` : ''}
                                </p>
                            </div>
                            ${statusBadge}
                        </div>
                        <div class="mt-3">
                            ${cls.st === 'Present' ? 
                                ...
                                `<button class="btn btn-primary btn-sm" onclick="initAtt('${cls.s}', this)">
    ...
    function updateAttendanceHistory() {
        ...
        if (todayClasses.length === 0) {
            historyList.innerHTML = `
                <div class="text-center p-5 text-muted">
                    ...
                    <p>No classes to track attendance</p>
                </div>`;
            return;
        }
        ...
        todayClasses.forEach(cls => {
            if (cls.st === 'Present') markedCount++;
            
            const statusIcon = cls.st === 'Present' ? 
                '<i class="bi bi-check-circle-fill text-success"></i>' :
                '<i class="bi bi-clock text-warning"></i>';
            
            const statusText = cls.st === 'Present' ? 'Marked' : 'Pending';
            ...
            html += `
                <div class="d-flex justify-content-between align-items-center p-3 border-bottom">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <p class="mb-0 fw-500">${cls.s}</p>
                            <small class="text-muted">${cls.t} • ${cls.r}</small>
                        </div>
                    </div>
                    <span class="badge ${cls.st === 'Present' ? 'bg-success' : 'bg-warning'}">${statusText}</span>
                </div>`;
        });
        ...
        const summary = `
            <div class="p-3 bg-light rounded-3 mb-3">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <p class="mb-1 text-muted small">Today's Attendance</p>
                        <h5 class="mb-0"><strong>${markedCount}/${totalCount}</strong> Classes Marked</h5>
                    </div>
                    <div class="text-end">
                        <div class="progress" style="width: 100px; height: 8px;">
                            <div class="progress-bar bg-success" style="width: ${totalCount > 0 ? (markedCount / totalCount) * 100 : 0}%"></div>
                        </div>
                        <small class="text-muted mt-1 d-block">${totalCount > 0 ? Math.round((markedCount / totalCount) * 100) : 0}%</small>
                    </div>
                </div>
            </div>`;

        historyList.innerHTML = summary + html;
    }
    ...
    function verifyLoc() {
        ...
        if (!navigator.geolocation) { showError("GPS not available"); return; }

        navigator.geolocation.getCurrentPosition(pos => {
            const dist = getDist(pos.coords.latitude, pos.coords.longitude, COLLEGE_LAT, COLLEGE_LNG);
            document.getElementById('distanceDisplay').innerText = `Distance: ${Math.round(dist)}m from campus`;

            if (dist <= ALLOWED_RADIUS) {
                spin.className = "bi bi-check-circle-fill text-success fs-5";
                txt.innerHTML = "<i class='bi bi-check-circle-fill me-1'></i>Step 1: Location Verified!";
                txt.className = "text-success fw-bold small";
                setTimeout(startFaceVerification, 1000);
            } else {
                spin.className = "bi bi-x-circle-fill text-danger fs-5";
                txt.innerHTML = "<i class='bi bi-x-circle-fill me-1'></i>Outside Campus Area";
                txt.className = "text-danger fw-bold small";
                showError(`Too far from campus (${Math.round(dist)}m). Please move closer.`);
            }
        }, () => showError("GPS access denied. Please enable location services."));
    }
    ...
    function startContinuousFaceDetection() {
        ...
        const detectionInterval = setInterval(() => {
            ...
            // Simple face detection by checking if video is playing
            if (video.readyState === video.HAVE_ENOUGH_DATA) {
                ...
                indicator.className = 'position-absolute top-0 end-0 m-2 badge bg-success';
                indicator.innerHTML = '<i class="bi bi-check-circle"></i> Ready to capture';
            }
        }, 500);
    ...
    window.captureFaceForVerification = async function() {
        const video = document.getElementById('videoElement');
        const canvas = document.getElementById('canvas');
        const ctx = canvas.getContext('2d');
        const captureBtn = document.getElementById('captureFaceBtn');
        ...
        try {
            ...
            captureBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Verifying...';
            statusDiv.innerHTML = '<span class="text-primary"><i class="bi bi-hourglass-split me-1"></i>Processing face verification...</span>';
            ...
            if (!faceVerificationService) {
                faceVerificationService = new FaceVerificationService();
                await faceVerificationService.initCamera('videoElement', 'canvas');
            }
            
            capturedFaceEncoding = faceVerificationService.generateFaceEncoding();
            
            // Send to backend for verification
            const response = await apiRequest('/face/verify', {
                method: 'POST',
                body: JSON.stringify({
                    faceEncoding: capturedFaceEncoding,
            ...
            if (response.success && response.verified) {
                faceVerified = true;
                statusDiv.innerHTML = `<span class="text-success"><i class="bi bi-check-circle-fill me-1"></i>Face verified successfully! (${Math.round(response.confidence * 100)}% match)</span>`;
                successMsg.classList.remove('d-none');
                successMsg.innerHTML = '<i class="bi bi-shield-check me-2"></i>All verifications complete! You can now submit attendance.';
                ...
                captureBtn.innerHTML = '<i class="bi bi-check-circle me-2"></i>Verified';
            ...
            statusDiv.innerHTML = `<span class="text-danger"><i class="bi bi-x-circle-fill me-1"></i>${error.message}</span>`;
            captureBtn.disabled = false;
            captureBtn.innerHTML = '<i class="bi bi-camera-fill me-2"></i>Try Again';
            showError(error.message || 'Face verification failed. Please try again.');
        }
    };

    async function startCam() {
        // Legacy function - now redirects to face verification
        await startFaceVerification();
    }

    window.takeSnapshot = function () {
        // Legacy function - now handled by captureFaceForVerification
        captureFaceForVerification();
    };

    window.finalizeAttendance = function () {
        if (!faceVerified) {
            showError('Please complete face verification first!');
            return;
        }
        ...
        r.querySelector('.loc-info').innerHTML = `<i class="bi bi-shield-check text-success"></i> Verified`;
        ...
        activityLog.unshift({ t: timeStr, msg: `Marked Present in ${subj} (Face Verified)` });
        ...
        updateActivityFeed();
        addNotification(`${subj} attendance marked with face verification at ${timeStr}`);
    ...
    window.closeCamera = function () {
        ...
        if (faceVerificationService) {
            faceVerificationService.stopCamera();
        }
        faceVerified = false;
        capturedFaceEncoding = null;
        modalObj.hide();
    ...
    window.handleLogout = async function (event) {
        ...
        // Disconnect sockets
        try { if (window.socketService) socketService.disconnect(); } catch {}
        // Clear local auth
        localStorage.removeItem('token');
        localStorage.removeItem('user');
        // Redirect
    ...
    // ==================== REAL-TIME ATTENDANCE MARKING FUNCTIONS ====================
    ...
    async function checkActiveAttendanceSession() {
        try {
            const response = await apiRequest('/attendance/active-session');

            if (response.hasActiveSession && response.sessions.length > 0) {
                const session = response.sessions[0];
                currentSession = session;

                // Show active session
                document.getElementById('noSessionMsg').style.display = 'none';
                document.getElementById('activeSessionAlert').style.display = 'block';
                document.getElementById('expectedSSID').textContent = session.wifiSSID;

                // Show step details
                document.getElementById('wifiDetailContainer').style.display = 'block';
                document.getElementById('locationDetailContainer').style.display = 'block';
                document.getElementById('faceDetailContainer').style.display = 'block';
                document.getElementById('submitDetailContainer').style.display = 'block';

                return true;
            } else {
                // No active session
                document.getElementById('noSessionMsg').style.display = 'block';
                document.getElementById('activeSessionAlert').style.display = 'none';
                document.getElementById('wifiDetailContainer').style.display = 'none';
                document.getElementById('locationDetailContainer').style.display = 'none';
                document.getElementById('faceDetailContainer').style.display = 'none';
                document.getElementById('submitDetailContainer').style.display = 'none';
    ...
    window.checkWiFiConnection = async function () {
        if (!currentSession) {
            showToast('No active session', 'warning');
            return;
        }

        const expectedSSID = currentSession.wifiSSID;
        const statusMsg = document.getElementById('wifiStatusMsg');
        statusMsg.innerHTML = '<div class="spinner-border spinner-border-sm me-2"></div>Checking WiFi...';

        try {
            // In a real browser, we'd use the WiFi API if available
            // For now, we'll make an API call to check connection
            const response = await apiRequest('/attendance/check-wifi', {
                method: 'POST',
                body: JSON.stringify({
                    sessionId: currentSession.sessionId,
                    ssid: expectedSSID, // In production, this would come from navigator.connection or similar
            ...
            if (response.wifiConnected) {
                ...
                document.getElementById('wifiStatus').className = 'badge bg-success ms-auto';
                document.getElementById('step-wifi').classList.add('border-success');
                statusMsg.innerHTML = '<div class="alert alert-success mb-0"><i class="bi bi-check-circle me-2"></i>WiFi connected successfully!</div>';

                // Emit socket event via service
                if (window.socketService && socketService.socket) {
                    socketService.emitWifiConnected({
                        sessionId: currentSession.sessionId,
                        studentId: user.id,
                ...
                statusMsg.innerHTML = `<div class="alert alert-danger mb-0"><i class="bi bi-exclamation-circle me-2"></i>${response.message}</div>`;
            }
        } catch (error) {
            console.error('WiFi check error:', error);
            statusMsg.innerHTML = '<div class="alert alert-warning mb-0"><i class="bi bi-exclamation-triangle me-2"></i>Failed to check WiFi. Make sure you\'re connected to the correct network.</div>';
        }
    };

    // Verify student location
    window.verifyStudentLocation = async function () {
        if (!currentSession || !wifiConnected) {
            showToast('Please connect to WiFi first', 'warning');
            return;
        }

        const statusMsg = document.getElementById('locationStatusMsg');
        statusMsg.innerHTML = '<div class="spinner-border spinner-border-sm me-2"></div>Verifying location...';
        ...
        try {
            ...
            // Call API to validate location
            const response = await apiRequest('/attendance/validate-location', {
                method: 'POST',
                body: JSON.stringify({
                    sessionId: currentSession.sessionId,
                    latitude,
            ...
            if (response.isWithinGeofence) {
                ...
                document.getElementById('locationStatus').textContent = `${response.distance}m away`;
                document.getElementById('locationStatus').className = 'badge bg-success ms-auto';
                document.getElementById('step-location').classList.add('border-success');
                statusMsg.innerHTML = `<div class="alert alert-success mb-0"><i class="bi bi-check-circle me-2"></i>Location verified! Distance: ${response.distance}m</div>`;

                // Emit socket event via service
                if (window.socketService && socketService.socket) {
                    socketService.emitLocationVerified({
                        sessionId: currentSession.sessionId,
                        studentId: user.id,
                        studentName: user.name,
                ...
                statusMsg.innerHTML = `<div class="alert alert-danger mb-0"><i class="bi bi-exclamation-circle me-2"></i>You're ${response.distance}m away. Required: within ${response.maxAllowedDistance}m</div>`;
            }
        } catch (error) {
            console.error('Location verification error:', error);
            statusMsg.innerHTML = '<div class="alert alert-warning mb-0"><i class="bi bi-exclamation-triangle me-2"></i>Failed to get location. Please enable GPS and try again.</div>';
        }
    };

    // Start face verification
    // Delegate to unified face verification flow
    window.startFaceVerification = async function () {
        if (!currentSession || !wifiConnected || !locationVerified) {
            showToast('Please verify WiFi and location first', 'warning');
            return;
        }
        // Use the unified handler defined earlier
        await startFaceVerification();
    };

    // Capture face image
    window.captureFaceImage = async function () {
        // Delegate to unified capture
        await captureFaceForVerification();
        // On success, emit via service
        if (faceVerified && window.socketService && socketService.socket) {
            socketService.emitFaceVerificationSuccess({
                sessionId: currentSession?.sessionId,
                studentId: user.id,
                studentName: user.name,
                confidence: 0.85
    ...
    window.stopFaceCamera = function () {
        // Delegate to unified close
        closeCamera();
    };

    // Submit attendance
    window.submitStudentAttendance = async function () {
        if (!currentSession || !wifiConnected || !locationVerified || !faceVerified) {
            showToast('Please complete all verifications first', 'warning');
            return;
        }
        ...
        try {
            const response = await apiRequest('/attendance/mark-real-time', {
                method: 'POST',
                body: JSON.stringify({
                    sessionId: currentSession.sessionId,
                    wifiConnected,
            ...
            if (response.success) {
                ...
                document.getElementById('submitStatus').className = 'badge bg-success ms-auto';

                // Show confirmation
                const submitMsg = document.getElementById('submitStatusMsg');
                submitMsg.innerHTML = `<div class="alert alert-success mb-0"><i class="bi bi-check-circle me-2"></i>${response.message}</div>`;
            ...
            const submitMsg = document.getElementById('submitStatusMsg');
            submitMsg.innerHTML = `<div class="alert alert-danger mb-0"><i class="bi bi-exclamation-circle me-2"></i>${error.message}</div>`;
    ...
    function resetAttendanceUI() {
        ...
        document.getElementById('wifiStatus').className = 'badge bg-secondary ms-auto';
        document.getElementById('locationStatus').textContent = 'Pending';
        document.getElementById('locationStatus').className = 'badge bg-secondary ms-auto';
        document.getElementById('faceStatus').textContent = 'Pending';
        document.getElementById('faceStatus').className = 'badge bg-secondary ms-auto';
        document.getElementById('submitStatus').textContent = 'Pending';
        document.getElementById('submitStatus').className = 'badge bg-secondary ms-auto';

        // Reset step styling
        ['step-wifi', 'step-location', 'step-face', 'step-submit'].forEach(step => {
            document.getElementById(step).classList.remove('border-success');
        });

        // Clear messages
        document.getElementById('wifiStatusMsg').innerHTML = '';
        document.getElementById('locationStatusMsg').innerHTML = '';
        document.getElementById('faceStatusMsg').innerHTML = '';
        document.getElementById('submitStatusMsg').innerHTML = '';
    ...
    window.addEventListener('load', () => {
        // Check for active session when attendance section is loaded
        const attendanceNav = document.querySelector('[data-section="attendance-section"]');
        if (attendanceNav) {
            attendanceNav.addEventListener('click', checkActiveAttendanceSession);
        }

        // Set up socket listeners if socket exists
        if (window.socketService && socketService.socket) {
            socketService.on('attendanceSessionActive', (data) => {
                currentSession = {
            ...
            socketService.on('sessionClosed', (data) => {
                currentSession = null;
                checkActiveAttendanceSession();
                showToast('Attendance session has ended', 'info');
            });

            socketService.on('attendanceMarkedSuccess', (data) => {
                showToast(data.message || 'Attendance marked successfully!', 'success');
                resetAttendanceUI();
            });

            socketService.on('attendanceMarkingFailed', (data) => {
                showToast(data.message || 'Failed to mark attendance', 'error');
            });

            socketService.on('locationVerificationFailed', (data) => {
                showToast(`Location verification failed: ${data.message}`, 'error');
                locationVerified = false;
                document.getElementById('locationStatus').textContent = 'Failed';
                document.getElementById('locationStatus').className = 'badge bg-danger ms-auto';
            });

            socketService.on('faceVerificationFailed', (data) => {
                showToast(`Face verification failed: ${data.message}`, 'error');
                faceVerified = false;
                document.getElementById('faceStatus').textContent = 'Failed';
                document.getElementById('faceStatus').className = 'badge bg-danger ms-auto';
            });
        }
    });

    // Periodically check for active session
    setInterval(checkActiveAttendanceSession, 5000);
