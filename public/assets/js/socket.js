...
class SocketService {
    ...
    connect() {
        ...
        if (typeof io === 'undefined') {
            console.warn('Socket.IO not loaded, skipping socket connection');
            return;
        }
    ...
    setupCommonListeners() {
        ...
        // Attendance session active broadcast
        this.socket.on('attendanceSessionActive', (data) => {
            this.emit('attendanceSessionActive', data);
        });
        ...
        this.socket.on('attendanceMarkedSuccess', (data) => {
            this.emit('attendanceMarkedSuccess', data);
        });
        this.socket.on('attendanceMarkingFailed', (data) => {
            this.emit('attendanceMarkingFailed', data);
        });

        // Verification failure forwards
        this.socket.on('locationVerificationFailed', (data) => {
            this.emit('locationVerificationFailed', data);
        });
        this.socket.on('faceVerificationFailed', (data) => {
            this.emit('faceVerificationFailed', data);
        });
    ...
    emitWifiConnected(data) {
        if (this.socket) {
            this.socket.emit('wifiConnected', data);
        }
    }
    ...
    emitFaceVerificationSuccess(data) {
        if (this.socket) {
            this.socket.emit('faceVerificationSuccess', data);
        }
    }

    emitFaceVerificationFailed(data) {
        if (this.socket) {
            this.socket.emit('faceVerificationFailed', data);
        }
    }
