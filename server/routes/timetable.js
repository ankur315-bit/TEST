...
router.get('/', protect, async (req, res) => {
    try {
        const { semester, branch, section, day, facultyId, batch } = req.query;
        ...
        if (req.user.role === ROLES.STUDENT) {
            // Build flexible query to match batch variations
            const userBatch = req.user.batch;
            const userBranch = req.user.branch || 'CSE';
            const userSemester = req.user.semester || 1;
            const userSection = req.user.section || 'A';
            
            // Match by exact batch name OR by branch/semester/section combination
            const batchQuery = {
                $or: []
            };
            
            if (userBatch) {
                batchQuery.$or.push({ batch: userBatch });
                batchQuery.$or.push({ batch: new RegExp(userBatch, 'i') });
            }
            
            // Also match branch + section patterns
            batchQuery.$or.push(
                { batch: `${userBranch}-${userSection}` },
                { batch: `${userBranch} ${userSection}` },
                { batch: new RegExp(`${userBranch}.*${userSection}`, 'i') }
            );
            
            query.$or = batchQuery.$or;
        }
...
router.post('/', protect, isAdmin, async (req, res) => {
    try {
        const { day, subject, subjectName, faculty, semester, branch, section, batch, classSection, startTime, endTime, room, type, teacher } = req.body;
        ...
        const conflict = await Timetable.findOne({
            ...
            $or: [
                { room, isActive: true },
                { faculty: faculty || teacher, isActive: true }
            ]
        });
        ...
        const entry = await Timetable.create({
            ...
            branch: branch || 'CSE',
            section: section || 'A',
            batch: batch || classSection,
            classSection: batch || classSection,
            startTime,
            endTime,
            room,
            type: type || 'lecture'
...
router.put('/:id', protect, isAdmin, async (req, res) => {
    try {
        const { day, subject, subjectName, faculty, teacher, semester, branch, section, batch, classSection, startTime, endTime, room, type } = req.body;
        ...
        // Update fields
        if (day) entry.day = day.toLowerCase();
        if (subject) entry.subject = subject;
        if (subjectName) entry.subjectName = subjectName;
        if (faculty || teacher) entry.faculty = faculty || teacher;
